...
function Home() {
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      try {
        const response = await productAPI.getTopSelling(1, 2024);
    ...
    datasets: [
      {
        label: "Doanh thu",
        data: [12, 19, 3, 5],
        backgroundColor: ["#AAC9FF", "#2A2A86", "#FCAF17", "#EF0D0D"],
    ...
    <div className={cx("container")}>
      <div className={cx("slogan-downloadbtn")}>
        <div className={cx("slogan")}>
          ...
          <span className={cx("slogan-sentence")}>
            Cháy cùng ngọn lửa đam mê
          </span>
        </div>
        <Button
          leftIcon={<StatisticIcon width={25} height={25} />}
          className={cx("statistic-btn")}
          primary
        >
          Xuất hình ảnh thống kê
        </Button>
          ...
          <div className={cx("content-wrapper")}>
            <div className={cx("chart-title")}>
              <span className={cx("title-text")}>
                Top 3 Nhóm sản phẩm bán chạy nhất tháng {getCurrentMonth()}
              </span>
              <Button
                primary
                rightIcon={<DetailIcon />}
                className={cx("detail-btn")}
                to={config.routes.statisticBestSeller}
              >
                Xem thêm
              </Button>
            </div>
            <div className={cx("chart")}>
              <BarChartItem data={dataBarChart} domain={[0, 20]} barSize={40} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={cx("second-labels")}>
        <div className={cx("first-statistic")}>
          <div className={cx("content-wrapper")}>
            <div className={cx("chart-title")}>
              <span>Doanh thu năm {getCurrentYear() - 1}</span>
              <Button
                primary
                rightIcon={<DetailIcon />}
                className={cx("detail-btn")}
                to={config.routes.statisticRevenueSales}
              >
                Xem thêm
              </Button>
            </div>
        ...
        <div className={cx("second-statistic")}>
          <div className={cx("chart-title")}>
            <span className={cx("title")}>
              Nhóm sản phẩm đem lợi nhuận cao nhất tháng {getCurrentMonth()}
            </span>
            <Button
              primary
              rightIcon={<DetailIcon />}
              className={cx("detail-btn")}
              to={config.routes.statisticrRevenueContributor}
            >
              Xem thêm
            </Button>
          </div>
