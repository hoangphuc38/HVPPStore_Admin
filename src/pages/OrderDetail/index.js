...
function OrderDetail() {
  ...
  const HandleChangeStatus = async () => {
    ...
    if (activeIndex < 4) {
      ...
      return await orderAPI
        ...
        .then((response) => {
          if (response.data.status === "Packaging") {
            HandleExportFile();
          }
  ...
  const renderStatusOrderFontStyle = (statusSelectedOrder) => {
    if (statusSelectedOrder === "Pending") {
      ...
      return (
        <span className={cx("status-lightblue")}>{statusSelectedOrder}</span>
      );
    ...
    <div className={cx("container")}>
      <div className={cx("order-status")}>
        ...
        <div className={cx("status-text")}>
          {renderStatusOrderFontStyle(statusSelectedOrder)}
        </div>
      </div>
            ...
            <div className={cx("detail-info")}>
              ...
              <div className={cx("detail")}>
                <span className={cx("info")}>
                  {orderDetail.name !== null ? orderDetail.name : "null"}
                </span>
                <span className={cx("info")}>
                  {orderDetail.phone !== null ? orderDetail.phone : "null"}
                </span>
                <span className={cx("info")}>
                  {orderDetail.address !== null ? orderDetail.address : "null"}
                </span>
              </div>
            </div>
          </div>
            ...
            item={
              <>
                {productInfo.map((product, index) => {
                  return <DetailProductItem data={product} key={index} />;
                })}
              </>
            }
          />
          ...
          <div className={cx("delivery-payment")}>
            ...
            <div className={cx("content")}>
              ...
              <div className={cx("content-detail")}>
                <span className={cx("sum-order")}>
                  {orderDetail.value !== null
                    ? orderDetail.value + " VND"
                    : "null"}
                </span>
                <span>
                  {orderDetail.deliveryMethod !== null
                    ? orderDetail.deliveryMethod
                    : "null"}
                </span>
                <span>
                  {orderDetail.payMethod !== null
